/*
  # Fix session_id ambiguity in keystroke verification

  1. Changes
    - Fix ambiguous session_id references in verify_keystroke_pattern function
    - Improve pattern comparison logic
    - Add better error handling
    - Optimize query performance with proper table aliases
*/
...
BEGIN
  ...
  WITH current_keystrokes AS (
    SELECT 
      array_agg(ARRAY[
        kd.dwell_time::float,
        COALESCE(kd.flight_time, 0)::float
      ] ORDER BY kd.created_at) as pattern
  ...
  WITH historical_sessions AS (
    SELECT DISTINCT ks.id as session_id
  ...
  -- Return average similarity score with additional context
  RETURN json_build_object(
    'match_score',
    CASE 
      ...
      ELSE 0.5
    END,
    'patterns_compared', pattern_count,
    'total_patterns', array_length(historical_patterns, 1)
  );
END;
$$;
